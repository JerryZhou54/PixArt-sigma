...
def make_tome_block(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    ...
    class ToMeBlock(block_class):
        ...
        def forward(self, x: torch.Tensor, context: torch.Tensor = None) -> torch.Tensor:
            ...
            x = x + self.drop_path1(u_a(self.ls1(self.attn(m_a(self.norm1(x))))))
            x = x + self.drop_path2(u_m(self.ls2(self.mlp(m_m(self.norm2(x))))))

            return x
    
    return ToMeBlock

def make_tome_block_pixart(block_class: Type[torch.nn.Module]) -> Type[torch.nn.Module]:
    """
    Make a patched class on the fly so we don't have to import any specific modules.
    This patch applies ToMe to the forward function of the block.
    """
    ...
    class ToMeBlock(block_class):
        ...
        def forward(self, x, y, t, mask=None, timestep=0, mask_dict=None, is_pad_token=None, T=0):
            B, N, C = x.shape
            # self._tome_info['size'] = (int(N**0.5), int(N**0.5))
            self._tome_info['size'] = (1, N)
            ...
            x = x + self.drop_path(u_a(gate_msa * self.attn(t2i_modulate(m_a(self.norm1(x)), shift_msa, scale_msa)).reshape(B, -1, C)))
            x = x + u_c(self.cross_attn(m_c(x), y, mask))
            x = x + self.drop_path(u_m(gate_mlp * self.mlp(t2i_modulate(m_m(self.norm2(x)), shift_mlp, scale_mlp))))
...
def apply_patch(
    ...
    if not is_diffusers:
        if hasattr(model, "blocks"):
            print("tome for encoder!")
            diffusion_model = model
        elif not hasattr(model, "model") or not hasattr(model.model, "diffusion_model"):
    ...
    for _, module in diffusion_model.named_modules():
        ...
        if isinstance_str(module, "Block"):
        ...
        elif isinstance_str(module, "PixArtBlock"):
            make_tome_block_fn = make_tome_block_pixart
            module.__class__ = make_tome_block_fn(module.__class__)
            module._tome_info = diffusion_model._tome_info
