...
def select_topk_and_remaining_tokens(x, token_weights, k, C):
    ...
    return topk_x, remaining_x, sorted_indices
...
class PixArtMSBlock(nn.Module):
    ...
    def __init__(self, hidden_size, num_heads, mlp_ratio=4.0, drop_path=0., input_size=None,
                 ...
                 # hr added
                layer_id=0, routing=False, capacity=0,
                **block_kwargs):
        super().__init__()
    ...
    def forward(self, x, y, t, mask=None, HW=None, **kwargs):
        ...
        '''
        x shape: 2 x 1024 x 1153
        k: 716
        shift_msa: 2 x 1 x 1153
        HW: 32 x 32
        y: 1 x 4 x 1152
        t: 2 x 6912
        '''
        ...
        if self.routing:
            ...
            topk_x, remaining_x, sorted_indices = select_topk_and_remaining_tokens(x, token_weights, k, C)

            topk_x = topk_x + self.drop_path(gate_mlp * self.mlp(t2i_modulate(self.norm2(topk_x), shift_mlp, scale_mlp)))
        ...
        if not self.routing:
            ...
            self.blocks = nn.ModuleList([
                PixArtMSBlock(
                    ...
                    sr_ratio=int(kv_compress_config['scale_factor']) if i in kv_compress_config['kv_compress_layer'] else 1,
                    qk_norm=qk_norm,
                    layer_id=i, routing=self.routing, capacity=self.capacity,
                )
                for i in range(depth)
            ])
