...
class MultiHeadCrossAttention(nn.Module):
    ...
    def forward(self, x, cond, mask=None, x_mask=None):
        ...
        k, v = kv.unbind(2)
        attn_bias = None
        # if mask is not None or x_mask is not None:
        #     if mask is None:
        #         mask = torch.ones_like(x_mask)
        #     if x_mask is None:
        #         x_mask = torch.ones(B, N)
        #     x_lengths = x_mask.sum(dim=1).int().tolist()
        #     attn_bias = xformers.ops.fmha.attn_bias.BlockDiagonalMask.from_seqlens(x_lengths, mask)
        if mask is not None:
...
class WindowAttention(Attention_):
    ...
    def forward(self, x, mask=None, layer_count=0, time_count=0):
        B, N, C = x.shape
        ...
        attn_bias = None
        # if mask is not None:
        #     lengths = mask.sum(dim=1).int().tolist()  # Assuming mask is [B, N] and sum gives the lengths
        #     attn_bias = xformers.ops.fmha.attn_bias.BlockDiagonalMask.from_seqlens(lengths, lengths)
        if mask is not None:
            attn_bias = torch.zeros([B * self.num_heads, q.shape[1], k.shape[1]], dtype=q.dtype, device=q.device)
            attn_bias.masked_fill_(mask.squeeze(1).repeat(self.num_heads, 1, 1) == 0, float('-inf'))
        x = xformers.ops.memory_efficient_attention(q, k, v, p=self.attn_drop.p, attn_bias=attn_bias)
        ...
        global prune_layers
        if use_edm and ((time_count-1)%20 >= N_skip or layer_count in prune_less_layers) and (layer_count in prune_layers):
            q = rearrange(q, "b n h d -> (b h) n d")
            k = rearrange(k, "b n h d -> (b h) n d")
            sim = einsum('b i d, b j d -> b i j', q, k) * (self.head_dim ** -0.5)
            if mask is not None:
                sim = sim.masked_fill(mask == 0, float('-inf'))
            sim = sim.softmax(dim=-1)

            M = rearrange(sim, "(b h) n d -> b h n d", h=self.num_heads)
            N = M.shape[-1]
            attn = M.mean(dim=1)
            ...
            mask_sel = mask_sel.scatter_(1, imp_inds, 1)
            ...
            mask_sim = mask_sim.scatter_(1, prune_inds, 1)
        ...
        x = self.proj_drop(x)
        if use_edm:
            return x, imp_inds, prune_inds, sim_inds
        else:
            return x
...
class CaptionEmbedder(nn.Module):
    ...
    def forward(self, caption, train, force_drop_ids=None):
        ...
        if (train and use_dropout) or (force_drop_ids is not None) or (self.uncond_prob == 1):
