...
def train():
    ...
    for epoch in range(start_epoch + 1, config.num_epochs + 1):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            t = torch.randint(0, train_diffusion.num_timesteps//2, (1,), device=clean_images.device)
            timesteps = t.repeat(bs)*2 + 1
            ...
            with accelerator.accumulate(model):
                ...
                loss_term = train_diffusion.router_training_losses(model, clean_images, timesteps, model_kwargs=dict(y=y, mask=y_mask, data_info=data_info))
                # loss = loss_term['loss'].mean()
                # mod_loss = loss_term['mod'].mean()
                loss = loss_term['mse'].mean()
                mod_loss = loss_term['l1_loss'].mean()
                loss = loss + args.l1 * mod_loss
                optimizer.zero_grad()
                model.zero_grad()
            ...
            #         import json
            #         with open(os.path.join(config.work_dir, f'ratios_0.3/ratios_step{global_step}.json'), 'w') as fp:
            ...
            if global_step % config.save_model_steps == 0:
                ...
                if accelerator.is_main_process:
                    ...
                    save_checkpoint(os.path.join(config.work_dir, 'checkpoints'),
                                    epoch=epoch,
                                    step=global_step,
                                    model=accelerator.unwrap_model(model).routers,
                                    optimizer=optimizer,
            ...
            #     model.train()
            if (global_step % 100 == 0 or (step + 1) == 1):
                scores = [model.module.routers[idx]() for idx in range(0, config.train_sampling_steps, 2)]
                if accelerator.is_main_process:
                    for tracker in accelerator.trackers:
                        import wandb
                        mask = format_image_to_wandb(config.train_sampling_steps//2 , model.module.depth*2, scores)
                        mask = wandb.Image(
                            mask
                        )
                        tracker.log({"validation": mask})
                    flush()
...
if __name__ == '__main__':
    ...
    model = build_model(config.model,
                        ...
                        **model_kwargs).eval()
    print(train_diffusion.timestep_map)
    raise Exception
    model.add_router(config.train_sampling_steps, train_diffusion.timestep_map)
    # if accelerator.is_main_process:
    ...
    from diffusion.data.builder import CoCoCaption
    ...
    train_dataset = CoCoCaption(transform)
    ...
    train_dataloader = DataLoader(
        train_dataset,
        batch_size=int(config.train_batch_size // get_world_size()),
        shuffle=False,
        sampler=sampler,
        num_workers=2,
        pin_memory=True,
    ...
    #     for module in model.modules():
    #         if hasattr(module, "diff_mod_ratio"):
    #             print(f"Module {i}: {module.diff_mod_ratio}")
    #             i += 1
    # accelerator.wait_for_everyone()
    # temp = 1/0
    model.float()
